...
2. Annotate and extract data

You can annotate the page with any tools (e.g. scrapely_) you like, for the demo purpose i just annotate the content manually. then mdr will be able to find the other data in the list::

    [1]: mappings, reverse_mappings, _ = mdr.extract(candidates[0])

the `reverse_mappings` is dict containing all the aligned elements. so you can find the corresponding element to the
data you anntotated.

e.g. you can find a demo page here_ generated by `demo.py`. the first row in color are anntotated manually, the rest are extracted by mdr.
